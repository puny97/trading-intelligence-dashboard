...
function marketStatus(): { label: string; cls: string } {
  const t = istHHMM(nowIST())
  if (t >= 900 && t < 915) return { label: '⏳ PRE-MARKET', cls: 'premarket-badge' }
  if (t >= 915 && t <= 1530)
    return {
      label: '🟢 MARKET OPEN',
...
export default function FyersDashboard() {
  ...
  const losers = [...quotes]
    ...
    .slice(0, 20)
  const buzzers = [...quotes].sort((a, b) => (b.volRatio ?? 0) - (a.volRatio ?? 0)).slice(0, 20)
  ...
  const avgGapUp = gapUp.length ? gapUp.reduce((s, g) => s + g.gapPct, 0) / gapUp.length : 0
  const avgGapDown = gapDown.length ? gapDown.reduce((s, g) => s + g.gapPct, 0) / gapDown.length : 0

  // ── Watchlist management ─────────────────────────────────────────────────
  const addSymbol = () => {
    let sym = addInput.trim().toUpperCase()
    if (!sym) return
    if (!sym.startsWith('NSE:') && !sym.startsWith('BSE:')) sym = `NSE:${sym}-EQ`
    if (!watchlist.includes(sym) && watchlist.length < 200) saveWatchlist([...watchlist, sym])
    setAddInput('')
  }
  const removeSymbol = (sym: string) => saveWatchlist(watchlist.filter(s => s !== sym))
    ...
    return (
      <div className="auth-screen">
        <div className="auth-card">
          ...
          <p>Login with your Fyers account to get real-time data for your 200-stock watchlist.</p>
          <a href="/api/auth/login" className="auth-btn">
            🔐 Login with Fyers
          </a>
          <div className="auth-note">
            <strong>Setup required:</strong> Create an app at <code>myapi.fyers.in</code> and add
  ...
  return (
    <div className="fyers-dashboard">
      ...
      <div className="fyers-tabs">
        {(['gainers', 'losers', 'buzzers', 'gappers', 'watchlist'] as Tab[]).map(t => (
          <button
            key={t}
      ...
      {activeTab === 'gappers' && (
        <div className="fyers-panel">
          ...
          <div className="gapper-meta">
            ...
            <span>
              ↓ Avg gap down: <b style={{ color: 'var(--red)' }}>{avgGapDown.toFixed(2)}%</b>
            </span>
          ...
          <div className="gap-filter-row">
            ...
            <div className="gap-threshold">
              ...
              <input
                ...
                step="0.1"
                value={gapThreshold}
                onChange={e => setGapThreshold(Math.max(0.1, parseFloat(e.target.value) || 1))}
              />
              <span>%</span>
            </div>
          </div>
          ...
          {quotes.length > 0 && gappers.length === 0 && (
            <div className="no-gappers">
              ...
              <span style={{ fontSize: 10 }}>
                Try lowering the minimum gap threshold, or check after 9:00 AM IST
              </span>
            </div>
          )}
          {gappers.map((g, i) => {
      ...
      {(activeTab === 'gainers' || activeTab === 'losers' || activeTab === 'buzzers') && (
        <div className="fyers-panel">
          ...
          {(activeTab === 'gainers' ? gainers : activeTab === 'losers' ? losers : buzzers).map(
            (q, i) => {
              const isUp = q.pChange >= 0
              const prev = prevQuotes.current.get(q.symbol)
              const priceFlash =
                prev && q.ltp !== prev.ltp ? (q.ltp > prev.ltp ? 'flash-up' : 'flash-down') : ''
              return (
                <div
                  key={q.symbol}
                ...
                >
                  ...
                  {activeTab === 'buzzers' && (
                    <div
                      className="col-vol"
                      style={{
                        color: (q.volRatio ?? 0) > 2 ? 'var(--accent)' : 'inherit',
                      }}
                    >
            ...
            {watchlist.map(sym => {
              ...
              return (
                <div key={sym} className="wl-chip">
                  <div onClick={() => setChartSymbol(sym)} style={{ cursor: 'pointer' }}>
                    <div className="wl-sym">{sym.replace('NSE:', '').replace('-EQ', '')}</div>
                    {q && (
                      <div
                        className="wl-ltp"
                        style={{ color: isUp ? 'var(--green)' : 'var(--red)' }}
                  ...
                  <button className="wl-remove" onClick={() => removeSymbol(sym)}>
                    ✕
                  </button>
                </div>
              )
