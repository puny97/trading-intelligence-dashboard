...
interface ClientAnalysis {
  ...
  dates: string[]
  lastAction: 'BUY' | 'SELL'
  status: 'DAY_TRADER' | 'ACCUMULATING' | 'DISTRIBUTING' | 'OVERNIGHT_LONG' | 'OVERNIGHT_SHORT'
...
function analyzeClients(deals: TrackedDeal[]): ClientAnalysis[] {
  const map = new Map<
    string,
    {
      byDate: Map<string, { buyQty: number; sellQty: number; buyCr: number; sellCr: number }>
      allDates: Set<string>
      lastAction: 'BUY' | 'SELL'
      lastDate: string
      totalBuyQty: number
  ...
  for (const d of deals) {
    ...
    } else if (d.deal_date === c.lastDate && d.deal_type === 'SELL') c.lastAction = 'SELL'
  ...
  for (const [client, c] of map.entries()) {
    ...
    const isNew = dates.length === 1 && dates[0] === allDealDates[allDealDates.length - 1]
...
export default function DealTracker() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    'all' | 'DAY_TRADER' | 'ACCUMULATING' | 'DISTRIBUTING' | 'OVERNIGHT_LONG' | 'OVERNIGHT_SHORT'
  >('all')
  ...
  const loadDeals = useCallback(async (symbol: string) => {
    ...
    setDeals([])
    try {
      const j = await fetch(`/api/tracker?symbol=${encodeURIComponent(symbol)}`).then(r => r.json())
  ...
  const loadClientDeals = useCallback(async (client: string) => {
    try {
      const j = await fetch(`/api/tracker/clients?client=${encodeURIComponent(client)}`).then(r =>
        r.json()
      )
  ...
  async function deleteBulk(type: 'symbol' | 'all' | 'before', symbol?: string) {
    setDeleting(true)
    try {
      const url =
        type === 'all'
      ...
      setDeleteMsg(
        type === 'all' ? 'All data deleted.' : `${symbol} deleted (${j.rowsDeleted} records).`
      )
      setDeleteModal(null)
      setSelected(null)
      loadSymbols()
  ...
  async function syncNow() {
    ...
    setSyncResult(null)
    try {
      const j = await fetch('/api/tracker/sync', { method: 'POST' }).then(r => r.json())
      if (j.error) {
        setSyncResult(`Error: ${j.error}`)
        return
      }
      setSyncResult(`✓ Saved ${j.saved} new deals, skipped ${j.skipped} duplicates (${j.asOnDate})`)
      loadSymbols()
      loadAutoSymbols()
    } catch (e: any) {
  ...
  const filteredAnalysis =
    filterStatus === 'all' ? clientAnalysis : clientAnalysis.filter(c => c.status === filterStatus)
  ...
  const filteredClients = clientProfiles.filter(
    c => !clientSearch || c.client_name.toLowerCase().includes(clientSearch.toLowerCase())
  )

  const autoSet = new Set(autoSymbols.map(s => s.symbol))
  const netQtyAll = deals.reduce(
    (s, d) => (d.deal_type === 'BUY' ? s + Number(d.quantity) : s - Number(d.quantity)),
    0
  )
  const netCrAll = deals.reduce(
    (s, d) => (d.deal_type === 'BUY' ? s + Number(d.value_cr) : s - Number(d.value_cr)),
    0
  )
    ...
    <div className="tracker-wrap">
      ...
      <div className="deals-header">
        ...
        <div className="deals-controls">
          ...
          <a href="/api/tracker/export" className="refresh-btn" style={{ textDecoration: 'none' }}>
            ⬇ EXPORT ALL
          </a>
          <button
            className="refresh-btn"
      ...
      {/* ── Main tabs ── */}
      <div className="deals-tabs" style={{ borderBottom: '2px solid var(--border)' }}>
        {(
          [
              ...
              return (
                ...
                >
                  <div className="tracker-sym-top">
                    ...
                    <span className={`tracker-net ${net >= 0 ? 'green' : 'red'}`}>
                      {net >= 0 ? '+' : ''}
                      {fmtCr(net)}
                    </span>
                  </div>
          ...
          <div className="tracker-main">
            {!selected ? (
              ...
              <>
                ...
                <div className="tracker-sym-header">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--muted)' }}>{selectedSym?.name}</div>
                    </div>
                    {!autoSet.has(selected) ? (
                      <button
                        ...
                        onClick={() => addAutoTrack(selected, selectedSym?.name || '')}
                      >
                        ⚙ AUTO-TRACK
                      </button>
                    ) : (
                  ...
                  <div className="tracker-stats-row">
                    ...
                    <div className="tracker-stat">
                      ...
                      <div className={`dc-val ${netQtyAll >= 0 ? 'green' : 'red'}`}>
                        {netQtyAll >= 0 ? '+' : ''}
                        {fmtQty(netQtyAll)}
                      </div>
                    </div>
                    <div className="tracker-stat">
                      ...
                      <div className={`dc-val ${netCrAll >= 0 ? 'green' : 'red'}`}>
                        {netCrAll >= 0 ? '+' : ''}
                        {fmtCr(netCrAll)}
                      </div>
                    </div>
                ...
                {/* Status filter */}
                <div className="gap-filter-row" style={{ borderBottom: '1px solid var(--border)' }}>
                  <span
                    style={{
                      fontSize: 9,
                      color: 'var(--muted)',
                      ...
                      >
                        ...
                        <div className="ca-header">
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              {c.isNew && <span className="new-badge">🆕 NEW ENTRY</span>}
                              {isBigPlayer && <span className="big-badge">🐳 BIG PLAYER</span>}
                            ...
                            >
                              ...
                              {c.appearances !== 1 ? 's' : ''}: {c.dates.join(' · ')}
                            </div>
                          </div>
                        ...
                        <div className="ca-flow">
                          <div className="ca-flow-item">
                            ...
                            <span className="deal-num green">{fmtQty(c.buyQty)}</span>
                          ...
                          <div className="ca-flow-item">
                            ...
                            <span className="deal-num red">{fmtQty(c.sellQty)}</span>
                            <span style={{ fontSize: 10, color: 'var(--muted)' }}>
                          ...
                          <div className="ca-flow-item">
                            ...
                            <span
                              style={{
                                fontSize: 10,
                                color: c.netCr >= 0 ? 'var(--green)' : 'var(--red)',
                                fontWeight: 600,
                              }}
                            >
                              {c.netCr >= 0 ? '+' : ''}
                        ...
                        >
                          <span>
                            ...
                            <b
                              style={{
                                color: c.lastAction === 'BUY' ? 'var(--green)' : 'var(--red)',
                              }}
                            >
                              {c.lastAction}
                            </b>
                ...
                <details className="raw-timeline-toggle">
                  <summary>📋 Raw Deal Timeline ({deals.length} records)</summary>
                  <div>
                    {deals.map((d, i) => {
                      ...
                      return (
                        <div key={d.id}>
                          ...
                          <div className={`tracker-deal-row ${isBuy ? 'deal-buy' : 'deal-sell'}`}>
                            <div className={`tracker-dot ${isBuy ? 'buy' : 'sell'}`} />
      ...
      {mainTab === 'clients' && (
        <div className="tracker-body">
          <div className="tracker-sidebar">
            ...
            <div className="tracker-sym-count">{filteredClients.length} clients tracked</div>
            {loading && (
              <div className="no-data">
                <div className="spinner" />
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="tracker-sym-meta">
                    ...
                    <span style={{ color: 'var(--accent)' }}>{Number(c.symbol_count)} stocks</span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    {syms.length > 4 && (
                      <span style={{ fontSize: 8, color: 'var(--muted)' }}>+{syms.length - 4}</span>
                    )}
                  </div>
                </div>
              )
          ...
          <div className="tracker-main">
            {!selectedClient ? (
              <div className="tracker-placeholder">
                ...
                <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 6 }}>
                  See which stocks they trade and their pattern across your tracked universe
                </div>
              </div>
            ) : (
              (() => {
                const profile = clientProfiles.find(c => c.client_name === selectedClient)
                const syms = profile?.symbols?.split(',').filter(Boolean) || []
                const net = Number(profile?.total_buy_cr || 0) - Number(profile?.total_sell_cr || 0)
                const clientDealsBySymbol = new Map<string, TrackedDeal[]>()
                clientDeals.forEach(d => {
                  if (!clientDealsBySymbol.has(d.symbol)) clientDealsBySymbol.set(d.symbol, [])
                ...
                return (
                  <>
                    <div className="tracker-sym-header">
                      ...
                      <div className="tracker-stats-row">
                        <div className="tracker-stat">
                          ...
                          <div className="dc-val accent">{Number(profile?.symbol_count || 0)}</div>
                        </div>
                        <div className="tracker-stat">
                          <div className="dc-label">Deals</div>
                          <div className="dc-val">{Number(profile?.total_deals || 0)}</div>
                        </div>
                        <div className="tracker-stat">
                        ...
                        <div className="tracker-stat">
                          <div className="dc-label">Net Flow</div>
                          <div className={`dc-val ${net >= 0 ? 'green' : 'red'}`}>
                            {net >= 0 ? '+' : ''}
                            {fmtCr(net)}
                          </div>
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {syms.map(s => {
                          ...
                          return (
                            ...
                            >
                              <span className="col-symbol" style={{ fontSize: 11 }}>
                                {s}
                              </span>
                              <span
                    ...
                    <div className="tracker-timeline">
                      ...
                      {clientDeals.map((d, i) => {
                        const isBuy = d.deal_type === 'BUY'
                        const prev = i > 0 ? clientDeals[i - 1].deal_date : null
                        return (
                          <div key={d.id}>
                            {d.deal_date !== prev && (
                              <div className="tracker-date-sep">
      ...
      {mainTab === 'autotrack' && (
        <div style={{ padding: 20 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 3 }}>
                Click <b style={{ color: 'var(--text)' }}>SYNC NOW</b> once a day after market close
                to auto-save all deals for tracked symbols.
                <br />
                Alternatively, each symbol gets auto-tracked when you click "AUTO-TRACK" button in
            ...
            >
              ...
              {syncResult && (
                <div
                  style={{
                    fontSize: 10,
                    color: syncResult.startsWith('Error') ? 'var(--red)' : 'var(--green)',
                  }}
                >
                  {syncResult}
                </div>
          ...
          >
            ...
            <br />
            1. Add symbols below (or click AUTO-TRACK button on any symbol in the Intelligence tab)
            <br />
            2. Every day after <b style={{ color: 'var(--text)' }}>market close (~3:45 PM IST)</b>,
            click SYNC NOW once
            <br />
            3. All bulk + block deals for your tracked symbols get saved automatically to Turso
            <br />
            ...
            {autoSymbols.map(s => (
              <div key={s.symbol} className="auto-sym-card">
                ...
                >
                  ...
                  {s.last_synced && (
                    <span>🔄 Synced: {new Date(s.last_synced).toLocaleDateString('en-IN')}</span>
                  )}
                </div>
              </div>
            ))}
            {autoSymbols.length === 0 && (
              ...
              >
                ...
                <span style={{ fontSize: 10 }}>
                  Go to Symbol Intelligence tab and click AUTO-TRACK on any symbol.
                </span>
              </div>
            )}
          </div>
          ...
          >
            ...
            {deleteMsg && (
              <div
                style={{
                  fontSize: 11,
                  color: deleteMsg.startsWith('Error') ? 'var(--red)' : 'var(--green)',
                  marginBottom: 12,
                }}
              >
                {deleteMsg}
        ...
        >
          <div className="chart-modal">
            <div className="chart-modal-header">
              ...
              <button className="chart-close" onClick={() => setChartSymbol(null)}>
                ✕
              </button>
            </div>
