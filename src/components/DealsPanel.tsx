...
export default function DealsPanel() {
  ...
  function sorted(deals: Deal[]): Deal[] {
    let d = [...deals];
    if (activeTab !== "short" && filterDir !== "all")
      d = d.filter((x) => x.dealType === filterDir);
    if (search) {
  ...
  const deals =
    activeTab === "bulk" ? bulk : activeTab === "block" ? block : short;
  const display = sorted(deals);
  const totalBuy = display
    .filter((d) => d.dealType === "BUY")
    .reduce((s, d) => s + d.value, 0);
  const totalSell = display
    .filter((d) => d.dealType === "SELL")
    .reduce((s, d) => s + d.value, 0);
  const netFlow = totalBuy - totalSell;
  const topSymbols = [
    ...new Map(display.map((d) => [d.symbol, d])).values(),
  ].slice(0, 5);
            ...
            <div className="deal-card deal-card-wide">
              ...
              >
                ...
                <b style={{ color: "var(--text)" }}>
                  symbol + shorted quantity
                </b>
                . Client names and prices are{" "}
                <b style={{ color: "var(--text)" }}>not published</b> by NSE for
                short selling — this is by regulation.
              </div>
            </div>
          </>
        )}
        ...
        {activeTab === "short" ? (
          ...
          <>
            ...
            <div
              ...
              style={{
                gridTemplateColumns:
                  "28px 1fr 1fr 70px 80px 90px 90px 70px 36px",
              }}
            >
              <span>#</span>
              <span>SYMBOL</span>
            ...
            {display.map((d, i) => (
              ...
              >
                ...
                <div>
                  <span
                    className={`deal-type-badge ${d.dealType === "BUY" ? "buy" : "sell"}`}
                  >
                ...
                <div
                  className={`deal-num deal-value ${d.dealType === "BUY" ? "green" : "red"}`}
                >
                  {fmtCr(d.value)}
                </div>
                <div className="deal-time">{d.date || "—"}</div>
