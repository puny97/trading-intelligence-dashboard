...
async function fetchDealsFromNSE(): Promise<{
  ...
  const r = await fetch(
    `${BASE}/api/snapshot-capital-market-largedeal?type=bulk_deals`,
    {
  ...
  const block = (json?.BLOCK_DEALS_DATA || []).map((d: any) =>
    norm(d, "block"),
  );
...
export async function POST() {
  ...
  try {
    ...
    const trackedSet = new Set(
      symResult.rows.map((r: any) => r.symbol as string),
    );

    if (trackedSet.size === 0)
      return NextResponse.json({
        message:
          "No symbols being tracked. Add symbols in Auto-Track tab first.",
        saved: 0,
        skipped: 0,
      });
    ...
    if (allDeals.length === 0)
      return NextResponse.json({
        message:
          "No deals found from NSE today. Try after market close (3:30 PM IST).",
        saved: 0,
        skipped: 0,
        asOnDate,
      });
    ...
    if (relevant.length === 0)
      return NextResponse.json({
        ...
        todaySymbols: [...new Set(allDeals.map((d: any) => d.symbol))].slice(
          0,
          20,
        ),
      });
